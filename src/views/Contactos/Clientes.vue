<template>
  <section class="side-content">
    ...
    <paginacion-item :paginas="paginas" :actual="page" name="clipage" />
  </section>
...
export default {
  ...
  methods: {
    async listarClientes(id) {
      let { error, lista, numeroDePaginas } = await contactos.allClients(id);
      this.loading = false;
      if (error) {
        this.error = error;
      } else {
    ...
    borrarCliente(id, nombre) {
      ...
      this.models = { id, mensaje: `¿Confirma borrar cliente: ${nombre}?` };
    },
    async accionBorrar({ id }) {
      this.$loader(document.querySelector("#ldelete"));
      let { error, mensaje } = await contactos.delete({ id, tipo: "clientes" });
      if (error) this.$toast.error(error);
      else {
    ...
    async formularioBuscar(info) {
      this.$loader(document.querySelector("#loader"));
      let { error, lista } = await contactos.formSearch(info, "clientes");
      if (error) this.$toast.error(error);
